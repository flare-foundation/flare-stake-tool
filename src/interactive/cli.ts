...
// import { ClaimCStakeRewardTxParams, TxDetails } from "../flare/interfaces";
  ...
  while (true) {
    ...
    if (Object.keys(taskConstants).slice(0, 4).includes(task.toString())) {
      if (
        ...
        walletProperties.wallet == Object.keys(walletConstants)[2] &&
    ...
    // Adding a validator
      ...
      if (walletProperties.wallet == Object.keys(walletConstants)[0] && fileExists('ctx.json')) {
        ...
        if (ctxNetwork && ctxDerivationPath && ctxPAddress && ctxCAddress) {
          //await checkAddressRegistrationLedger(
        ...
        if (ctxNetwork && ctxVaultId && ctxPublicKey) {
          ...
          if (!isContinue.isContinue) {
            ...
            if (isRegistered) {
              ...
              throw Error('Address not registered')
            }
            //else {
            //txnId = await registerAddressForDefi(
            //  walletProperties.wallet,
            //  ctxNetwork,
            //  ctxPublicKey,
            //);
        ...
        //await checkAddressRegistrationPrivateKey(
        //  walletProperties.wallet,
        //  walletProperties.network!,
        //  walletProperties.path!,
        //);
    ...
    //    const isUnclaimed = await isUnclaimedReward(
    //      context.cAddressHex!,
    //      context.config.hrp,
    //    );
    //    if (isUnclaimed) {
    ...
    //    walletProperties.path
    //  ) {
    //    const context: Context = contextEnv(
    //      walletProperties.path,
    //      walletProperties.network,
    //    );
    //    const isUnclaimed = await isUnclaimedReward(
    ...
    //    );
    //    if (isUnclaimed)
    //      await claimRewardsPrivateKey(walletProperties.wallet, context);
    //  }
    //}
    ...
    // Withdraw funds
      ...
      if (walletProperties.wallet == Object.keys(walletConstants)[1] && fileExists('ctx.json')) {
        ...
        throw new Error(
          'Only Defi wallet supported for withdrawal, otherwise this is a standard ethereum transaction, for which you can use metamask.'
        )
      }
    ...
    // Optout
      ...
      if (walletProperties.wallet == Object.keys(walletConstants)[0] && fileExists('ctx.json')) {
        ...
        const argsOptOut = [
          ...
          `--network=${ctxNetwork}`,
          '--ledger',
          `--derivation-path=${ctxDerivationPath!}`
        ]
        try {
...
//async function checkAddressRegistrationLedger(
//  wallet: string,
//  ctxNetwork: string,
...
//async function checkAddressRegistrationPrivateKey(
//  wallet: string,
//  ctxNetwork: string,
//  pvtKeyPath: string,
//) {
...
//async function registerAddressForDefi(
//  wallet: string,
//  ctxNetwork: string,
//  ctxPublicKey: string,
//): Promise<string> {
...
//  const receiverAddress = isOwnerReceiver.isOwnerReceiver
//    ? ctx.cAddressHex!
//    : await getPromptsAddress();
...
//    ? ctxCAddress
//    : await getPromptsAddress();
//  const claimRewardsParams: ClaimCStakeRewardTxParams = {
...
//    return await signTransaction(request.unsignedTxHex, wallet);
//  };
//  console.log("Please sign the transaction on your ledger");
//  await flare.claimCStakeReward(claimRewardsParams, sign);
//  console.log(chalk.green("Rwards successfully claimed"));
//}
