  ...
  program
    ...
    .action(async (type: string, options: OptionValues) => {
      ...
      if (options.getHacked) {
        ...
        await cliBuildAndSaveUnsignedTxJson(type, ctx, options.transactionId as string, options as FlareTxParams);
      }
    });
...
async function buildUnsignedTx(transactionType: string, ctx: Context, params: FlareTxParams): Promise<UnsignedTx> {
  if (!ctx.cAddressHex) {
        ...
        transferTx = pvm.e.newBaseTx(
          {
            ...
            outputs: [
              TransferableOutput.fromNative(context.avaxAssetID, BigInt(params.amount), [recipientPAddressBytes]),
            ],
            utxos,
          },
          context
...
async function signForDefi(transaction: string, ctx: string, evmTx: boolean = false): Promise<void> {
