...
export function toHex(value: string | number | number[] | Buffer | Uint8Array, prefix0x: boolean = true): string {
  let hex;
  if (!value) {
    value = "0";
  }
...
export function gweiToFlr(gweiValue: number | BN | string, thousandsSeparator: boolean = false): string {
...
import { ContextFile, Context, UnsignedTxJson, SignedTxJson } from "./interfaces";
...
export function saveUnsignedTxJson(unsignedTxJson: UnsignedTxJson, id: string, dir?: string): string {
  if (dir === undefined) {
