...
import {
  ...
  // getDistributionToDelegatorsABI,
...
export async function isAddressRegistered(
  ...
  const contract = new ethers.Contract(
    addressBinderContractAddress,
    abi,
    provider,
  );

  const result = await contract.cAddressToPAddress(ethAddressToCheck);
  const pChainAddress = bech32.encode(
    network,
    bech32.toWords(Buffer.from(result.slice(2), "hex")),
  );

  if (result !== "0x0000000000000000000000000000000000000000") {
    console.log(
      chalk.green(
        `Address associated with key ${ethAddressToCheck}: ${pChainAddress}`,
      ),
    );
    return true;
  } else {
 ...
 * @param network
 */
export async function isUnclaimedReward(
  ethAddressToCheck: string,
  network: string,
): Promise<boolean> {
  console.log("Checking your Rewards status...");
  const rpcUrl = rpcUrlFromNetworkConfig(network);
  const validatorRewardManagerContractAddress = await getContractAddress(
    network,
    validatorRewardManagerContractName,
  );
  const provider = new ethers.providers.JsonRpcProvider(rpcUrl);
  ...
  const contract = new ethers.Contract(
    validatorRewardManagerContractAddress,
    abi,
    provider,
  );
  ...
  const unclaimedRewards: BigNumber =
    totalRewardNumber.sub(claimedRewardNumber);

  if (unclaimedRewards.gt(BigNumber.from("0"))) {
    const unclaimedRewardsInFLR: BigNumber = unclaimedRewards.div(
      ethers.constants.WeiPerEther,
    ); // div by 1e18
    console.log(
      chalk.green(
        `You have unclaimed rewards worth ${unclaimedRewardsInFLR} FLR`,
      ),
    );
    return true;
  } else {
...
//
//  const pAddr = prefix0x(
//    Buffer.from(
//      bech32.fromWords(bech32.decode(pAddress.slice(2)).words),
//    ).toString("hex"),
//  );
...
//  } = rewardsParams;
//
//  const rewardAmount: BigNumber = BigNumber.from(claimAmount).mul(
///    ethers.constants.WeiPerEther,
//  );
//  if (
//    rewardAmount.gt(await getUnclaimedRewards(ownerAddress, network)) ||
 ...
 * @returns TxnHash
 */
export async function submitForDefiTxn(
  id: string,
  signature: string,
  network: string,
): Promise<string> {
  const provider = new ethers.providers.JsonRpcProvider(
    rpcUrlFromNetworkConfig(network),
  );
  const serializedSignedTx = ethers.utils.serializeTransaction(
    readUnsignedEVMObject(id),
    prefix0x(signature),
  );
...
async function getContractAddress(
  ...
  if (network != "flare" && network != "costwo")
...
//   fs.mkdirSync(`${forDefiDirectory}/${forDefiUnsignedTxnDirectory}`, {
//     recursive: true,
//   });
...
// ): Promise<boolean> {
//   const rpcUrl = rpcUrlFromNetworkConfig(network);
//   const distributionToDelegatorsContractAddress = await getContractAddress(
//     network,
//     distributionToDelegators,
//   );
//   const provider = new ethers.providers.JsonRpcProvider(rpcUrl);
...
//   const contract = new ethers.Contract(
//     distributionToDelegatorsContractAddress,
//     abi,
//     provider,
//   );
//
...
//async function signContractTransaction(
...
//  const { cAddress, network, wallet, derivationPath, pvtKey, transactionId } =
//    params;
...
//  let gasEstimate;
//  try {
//    gasEstimate = await contract.estimateGas.optOutOfAirdrop({
//      from: cAddress,
//    });
//  } catch {
//    console.log(
//      chalk.red("Unable to estimate trx gas fees, Trx is likely to fail"),
//    );
//    exit();
//  }
//  const gasPrice = await provider.getGasPrice();
