...
export async function getAccount(
  ...
  let app = await _getApp();
  if (app === FLARE) {
    return await flare.getAccount(bip44Path, hrp);
  } else if (app === AVALANCHE) {
    return await avalanche.getAccount(bip44Path, hrp);
  } else if (app === ETHEREUM) {
    return await ethereum.getAccount(bip44Path);
  }
...
export async function verifyPAddress(
  bip44Path: string,
  hrp: string,
): Promise<string> {
  let app = await _getApp();
...
export async function sign(bip44Path: string, tx: string): Promise<string> {
  ...
  if (app === FLARE) {
    ...
    throw new Error(
      "Nonblind signing of transactions is not supported on Avalanche app",
    );
  } else if (app === ETHEREUM) {
    throw new Error(
      "Nonblind signing of transactions is not supported on Ethereum app",
    );
  } else {
    return "";
  }
...
export async function signHash(
  ...
  if (app === FLARE) {
    ...
    return await ethereum.signPersonalMessage(
      bip44Path,
      utils.toHex(message, false),
    );
  } else {
    return "";
  }
