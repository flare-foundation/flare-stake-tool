...
export async function getAccount(
  ...
  let publicKey = pubk.normalizePublicKey(utils.toHex(account!.compressed_pk));
  return { publicKey: publicKey, address: account!.bech32_address };
}
...
export async function getPAddress(
  ...
  if (account!.errorMessage && account!.errorMessage != "No errors") {
    throw Error(
      `Failed to obtain P-chain address from ledger: ${account!.errorMessage}, code ${account!.returnCode}`,
    );
  }
  return account!.address;
}
...
export async function signHash(
  ...
  await _connect(async (app) => {
    response = await (app as FlareApp).signHash(bip44Path, messageBuffer);
  });
  return _getSignatureFromResponse(response);
}

async function _connect(
  execute: (app: FlareApp) => Promise<void>,
): Promise<void> {
  let flare;
  try {
...
function _getSignatureFromResponse(response: any): string {
  if (!response) {
  ...
  let signature = Buffer.concat([response.r, response.s, response.v]).toString(
    "hex",
  );
  return signature;
}
