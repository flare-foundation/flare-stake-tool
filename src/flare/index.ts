...
export async function getAccount(
  ...
  if (pubk.isPublicKey(accountId)) {
    ...
    if (publicKey) {
      ...
      throw new Error(
        "Failed to determine stake account with the given C-chain address",
      );
    }
    ...
    if (publicKey) {
      ...
      throw new Error(
        "Failed to determine stake account with the given P-chain address",
      );
    }
  } else if (accountId.startsWith("NodeID-")) {
    let stakes = await chain.getPStakes(network);
    let stake = stakes.find(
      (s) => s.nodeId === accountId && s.type === "validator",
    );
    ...
    if (publicKey) {
      ...
      throw new Error(
        "Failed to determine stake account with the given node id",
      );
    }
  }
  throw new Error(
    "Failed to determine stake account with the given identifier",
  );
}
...
export async function getAccountState(
  ...
  let pStakeUTXOsOf = await chain.getUTXOsFromPStakes(
    network,
    account.pAddress,
    pStakesOf,
  );
...
async function _importCP(
  ...
  if (tx.submitted) {
    await _waitForBalanceChange(balance, () =>
      chain.getPBalance(account.network, account.pAddress),
    );
  }
  return tx;
}
...
async function _importPC(
  ...
  if (tx.submitted) {
    await _waitForBalanceChange(balance, () =>
      chain.getCBalance(account.network, account.cAddress),
    );
  }
  return tx;
}

async function _waitForBalanceChange(
  startBalance: BN,
  balance: () => Promise<BN>,
): Promise<void> {
...
async function _checkNumberOfStakes(
  ...
  let allStakesOf = await chain.getPStakesOf(
    account.network,
    account.pAddress,
    stakes,
  );
  ...
  let nodeIds = Array.from(
    new Set(stakesOf.map((s) => s.nodeId.toLowerCase())),
  );
...
async function _waitForStake(account: Account, txId: string): Promise<void> {
  await utils.waitWhile(
    async () =>
      (await chain.getPStakesOf(account.network, account.pAddress)).some(
        (x) => x.txId === txId,
      ),
    TX_EFFECT_TIMEOUT,
    TX_EFFECT_DELAY,
...
export async function wrapC(
  ...
  if (tx.submitted && !tx.confirmed) {
    throw new Error(
      `Wrap transaction ${tx.id} on C-chain not confirmed (status is ${tx.status})`,
    );
  }
  return tx.id;
}
...
export async function unwrapC(
  ...
  if (tx.submitted && !tx.confirmed) {
    throw new Error(
      `Unwrap transaction ${tx.id} on C-chain not confirmed (status is ${tx.status})`,
    );
  }
  return tx.id;
}
...
export async function submitTxHex(txHex: string): Promise<string> {
  ...
  if (result == null) {
    ...
    throw new Error(
      `Transaction ${result[0]} not confirmed (status is ${result[1]})`,
    );
  }
  return result[0];
}
