...
export async function buildExportCTx(
  ...
  if (params.exportFee.isZero()) {
    params.exportFee = await _getExportCTxFee(
      account,
      params.amount,
...
async function _getExportCTxFee(
  ...
  const tx = await _getUnsignedExportCTx(
    account,
    amount,
    new BN(0),
    importFeeReservation,
...
async function _getUnsignedImportCTx(
  account: Account,
  importFee: BN,
): Promise<UnsignedTx> {
  const context = await getContext(account.network);
...
export async function buildExportPTx(
  ...
  if (amount.isZero()) {
    amount = (await chain.getPBalance(account.network, account.pAddress)).sub(
      exportFee,
    );
  }
...
export async function buildClaimCStakeRewardTx(
  ...
  let rewardManager = contracts.getValidatorRewardManager(
    account.network,
    web3,
  );
...
export async function buildERC20TransferCTx(
  ...
  let data = (erc20.methods.transfer as any)(
    params.recipient,
    value,
  ).encodeABI() as string;
  ...
  if (signatureResponse.startsWith("id:")) {
    ...
    if (!pubk.equalPublicKey(publicKey, unsignedTxData.txDetails.publicKey)) {
      if (unsignedTx instanceof UnsignedTx) {
        publicKey = pubk.recoverPublicKeyFromEthMsg(
          utils.toHex(unsignedTxHash, false),
          signature,
        );
        if (
          !pubk.equalPublicKey(publicKey, unsignedTxData.txDetails.publicKey)
        ) {
          throw new Error(
    ...
    if (
      unsignedTx instanceof EvmLegacyTx ||
      unsignedTx instanceof EvmEIP1559Tx
    ) {
      ...
      const compressedPublicKey = pubk.compressPublicKey(
    ...
    if (!presubmit || (await presubmit(txSummary))) {
      if (
        ...
        if (unsignedTx instanceof EVMUnsignedTx) {
          ...
          throw new Error(
            `Can not issue transaction of type ${typeof unsignedTx}`,
          );
        }
      }
      submitted = true;
...
export async function submitTxHex(
  txHex: string,
...
async function _waitForEvmTxConfirmation(
  web3: Web3,
  txId: string,
): Promise<[string, boolean]> {
  let status = "Unkown";
